 ...
 */
    ...
    for (size_t iinstr = 0; iinstr < orchestra->n_instruments; iinstr++) {
        for (size_t inote = 0; inote < orchestra->n_keys_per_instrument[iinstr]; inote++) {
            if (orchestra->new_played_notes[iinstr][inote]) {
                add_frequency_components_for_a_note(current_note, note_fft_len, orchestra->additive_synths_coeffs[iinstr], 
                                                    orchestra->n_coeffs_per_instrument[iinstr], inote);
            }
        }
    }    
